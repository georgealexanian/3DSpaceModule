...
//this script helps move the Sun, the Moon and Earth along their X axis when the 
...
public class Xaxis : MonoBehaviour
{
    ...
    void OnMouseDrag()
    {
        ...
        if (SimulationMode.IsGameModeManual() && !UIElementTouchChecker.IsPointerOverUIElement() && canChangePos && Input.GetMouseButton(0))
        {
            ...
            transform.parent.parent.position = new Vector3(cursorPosition.x, transform.parent.parent.position.y, transform.parent.parent.position.z);
        }
        else
        {
            canChangePos = false;
