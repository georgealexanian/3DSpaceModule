...
public class BodyMovementController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (SimulationMode.IsGameModeManual() && setupViewAfterEclipseChosen.shouldSetupSpaceToSimulateEclipse == false && !UIElementTouchChecker.IsPointerOverUIElement() && movementOrRotationModeController.rotationOrMovementMode == MovementOrRotationModeController.RotationOrMovementMode.movement)
        {
                ...
                if (raycastHit.collider.tag == "Earth")
                {
                    ...
                    foreach (Transform axis in raycastHitTransform)
                    {
                        ...
                        if (axis.name == "Axes")
                        {
                            ...
                            GameObject earth = axis.parent.FindChild("EarthAroundAxis").gameObject; 
