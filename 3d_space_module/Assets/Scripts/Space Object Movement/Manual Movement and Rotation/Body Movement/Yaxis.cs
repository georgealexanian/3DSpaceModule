...
//this script helps move the Sun, the Moon and Earth along their Y axis when the 
...
public class Yaxis : MonoBehaviour
{
    ...
    void OnMouseDrag()
    {
        ...
        if (SimulationMode.IsGameModeManual() && !UIElementTouchChecker.IsPointerOverUIElement() && canChangePos && Input.GetMouseButton(0))
        {
            ...
            transform.parent.parent.position = new Vector3(transform.parent.parent.position.x, cursorPosition.y, transform.parent.parent.position.z);
        }
        else
        {
            canChangePos = false;
