...
namespace WPM
{
	...
	public class WorldMapEditorInspector : Editor
	{
		...
		GUIStyle labelsStyle, editorCaptionLabelStyle;
		...
		void ShowEntitySelectors ()
		{
			...
			if (_editor.countryIndex >= 0) {
				...
				if (citySelection != _editor.GUICityIndex) {
					...
					if (_editor.cityIndex >= 0)
						FocusSpherePoint (_map.cities [_editor.cityIndex].unitySphereLocation);
				}
				EditorGUILayout.EndHorizontal ();
				if (_editor.cityIndex >= 0) {
					EditorGUILayout.BeginHorizontal ();
			...
			if (_editor.countryIndex >= 0) {
				...
				if (mpSelection != _editor.GUIMountPointIndex) {
					...
					if (_editor.mountPointIndex >= 0)
						FocusSpherePoint (_map.mountPoints [_editor.mountPointIndex].unitySphereLocation);
				}
				EditorGUILayout.EndHorizontal ();
				if (_editor.mountPointIndex >= 0) {
					EditorGUILayout.BeginHorizontal ();
		...
		void ShowReshapingRegionTools ()
		{
			...
			if (_editor.reshapeRegionMode.hasCircle ()) {
				...
				_editor.reshapeCircleWidth = EditorGUILayout.Slider (_editor.reshapeCircleWidth, 0.0001f, 0.01f);
				EditorGUILayout.EndHorizontal ();
			}
			...
			case RESHAPE_REGION_TOOL.SPLITV:
				EditorGUILayout.BeginHorizontal ();
				GUILayout.Label ("Split At", GUILayout.Width (90));
		...
		static void RestoreBackup (MenuCommand command)
		{
			...
			string geodataFolderName = "Geodata";
			if (globe != null) {
				geodataFolderName = globe.geodataFolderName;
			}
			for (int k = 0; k < paths.Length; k++) {
				if (paths [k].EndsWith ("Resources/" + geodataFolderName)) {
				...
				if (mouseDown) {
					...
					if (selected) {
						...
						if (_editor.entities != null && _editor.entityIndex >= 0 && _editor.entityIndex < _editor.entities.Length)
							_editor.cursor = _editor.entities [_editor.entityIndex].sphereCenter;
					}
				}
			...
			for (int i = 0; i < points.Length; i++) {
				...
				newPoint = Handles.FreeMoveHandle (oldPoint, frontFaceQuaternion, handleSize, pointSnap,   
					(handleControlID, position, rotation, size, eventType) => {
						...
						Handles.DotHandleCap (handleControlID, position, rotation, size, eventType); // rotation, size);
					});
#else
				newPoint = Handles.FreeMoveHandle (oldPoint, frontFaceQuaternion, handleSize, pointSnap,   
		...
		bool SaveChanges ()
		{
			...
			if (_editor.cityChanges) {
				...
				fullPathName = GetAssetsFolder () + geoDataFolder + "/" + dataFileName;
				File.WriteAllText (fullPathName, _editor.GetCityGeoData (), Encoding.UTF8);
			...
			if (_editor.mountPointChanges) {
				...
				fullPathName = GetAssetsFolder () + geoDataFolder + "/" + dataFileName;
				File.WriteAllText (fullPathName, _editor.GetMountPointsGeoData (), Encoding.UTF8);
		...
		void ShowCandidatePoint ()
		{
			...
			for (int p = 0; p < max; p++) {
				...
				float dist = (rp - latlonCursor).sqrMagnitude; // (rp.x - _editor.cursor.x) * (rp.x - _editor.cursor.x) * 4 + (rp.y - _editor.cursor.y) * (rp.y - _editor.cursor.y);
				if (dist < minDist) {
					// Get nearest point
					minDist = dist;
					nearest = p;
			...
			if (hits.Length > 0) {
				for (int k = 0; k < hits.Length; k++) {
					if (hits [k].collider.gameObject == _map.gameObject) {
						...
						if (snapRequested) {
							if (_editor.GetVertexNearSpherePos (newPos, out newPos)) {
								mouseDown = true;
							}
						}
