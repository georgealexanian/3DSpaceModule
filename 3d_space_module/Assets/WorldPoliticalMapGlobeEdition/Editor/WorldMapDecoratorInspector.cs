...
namespace WPM {
	...
	public class WorldMapDecoratorInspector : Editor {
				...
				if (!decorator.hidden) {
					...
					if (decorator.labelVisible) {
						...
						if (decorator.labelOverridesColor) {
							...
							decorator.labelColor = EditorGUILayout.ColorField ("Color", decorator.labelColor);
					...
					if (decorator.isColorized) {
						...
						decorator.fillColor = EditorGUILayout.ColorField ("Fill Color", decorator.fillColor);
		...
		void ToggleZoomState () {
			zoomState = !zoomState;
			Camera cam = _map.mainCamera;
			if (zoomState) {
				oldCameraPos = cam.transform.position;
				cam.transform.position = _map.transform.position + (cam.transform.position - _map.transform.position) * _map.transform.localScale.z * 0.6f;
			} else {
