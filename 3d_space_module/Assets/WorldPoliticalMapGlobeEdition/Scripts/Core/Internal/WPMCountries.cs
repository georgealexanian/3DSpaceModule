...
namespace WPM {
	public partial class WorldMapGlobe : MonoBehaviour {
		...
		void ReadCountriesPackedString () {
			...
			string frontiersFileName = _frontiersDetail == FRONTIERS_DETAIL.Low ? "/countries110" : "/countries10";
			TextAsset ta = Resources.Load<TextAsset> (geodataFolderName + frontiersFileName);
			string s = ta.text;
			...
			for (int k = 0; k < countryCount; k++) {
				...
				if (countryInfo.Length >= 9) {
					country.labelVisible = countryInfo [8].Equals ("1"); 
				}
		...
		void RefreshCountryGeometry (Country country) {
		...
		void DrawMapLabels () { 
			...
			textRoot.transform.rotation = Misc.QuaternionZero; //Quaternion.Euler (Misc.Vector3zero);		  // needs rotation to be 0,0,0 for getting correct bounds sizes - it's fixed at the end of the method
			...
			textRoot.transform.localRotation = Misc.QuaternionZero; //Quaternion.Euler (Misc.Vector3zero);
...
//																								tmLabel.transform.localScale = new Vector3 (-tmLabel.transform.localScale.x * transform.localScale.x / 100f, tmLabel.transform.localScale.y * transform.localScale.y / 100f, tmLabel.transform.localScale.z);
		...
		bool GetCountryUnderMouseInt (Vector3 spherePoint, out int countryIndex, out int regionIndex) {
			...
			for (int c = 0; c < cc; c++) {
				...
				if (country.hidden || !country.regionsRect2D.Contains (mousePos))
					continue;
		...
		GameObject HighlightCountryRegionSingle (int countryIndex, int regionIndex, bool refreshGeometry, bool drawOutline, Color outlineColor) {
			...
			if (_countryHighlightMaxScreenAreaSize < 1f) {
				...
				doHighlight = CheckScreenAreaSizeOfRegion (region, _countryHighlightMaxScreenAreaSize);
			}
			if (doHighlight) {
				if (existsInCache) {
				...
				obj = GenerateCountryRegionSurface (countryIndex, regionIndex, hudMatCountry, Misc.Vector2one, Misc.Vector2zero, 0, drawOutline, outlineColor, true);
			}
			return obj;
		}

		bool CheckScreenAreaSizeOfRegion (Region region, float threshold) {
			Camera cam = mainCamera;
		...
		GameObject GenerateCountryRegionSurface (int countryIndex, int regionIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool drawOutline, Color outlineColor, bool temporary) {
			...
			int cacheIndex = GetCacheIndexForCountryRegion (countryIndex, regionIndex); 
//			GameObject surf = GenerateCountryRegionSurfaceOne (surfacesLayer.transform, cacheIndex.ToString (), countryIndex, regionIndex, material, textureScale, textureOffset, textureRotation, drawOutline, outlineColor, temporary);
		...
		GameObject DrawCountryRegionOutline (Region region, GameObject surf, Color outlineColor) {
			...
			boldFrontiers.transform.localRotation = Misc.QuaternionZero; //Quaternion.Euler (Misc.Vector3zero);
