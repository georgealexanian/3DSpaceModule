	...
	public partial class WorldMapGlobe : MonoBehaviour {
		...
		void CheckTiles (TileInfo parent, int currentZoomLevel, int xTile, int yTile, int zoomLevel, int subquadIndex) {
			...
			for (int c = 0; c < 4; c++) {
				...
				if (cornersOccluded) {
					float radiusSqr = (wpos.x - globePos.x) * (wpos.x - globePos.x) + (wpos.y - globePos.y) * (wpos.y - globePos.y) + (wpos.z - globePos.z) * (wpos.z - globePos.z);
			...
			if (!cornersOccluded) {
				...
				if (!insideViewport) {
					Bounds bounds = new Bounds (tileMidPoint, maxWorldPos - minWorldPos);
		...
		void CreatePole (TileInfo ti) {
			...
			Renderer obj = CreateObject (tilesRoot.transform, name, vertices.ToArray (), indices.ToArray (), uv.ToArray (), ti.parent.opaqueMat, 0);
			if (isNorth) {
				northPoleObj = obj;
			} else {
				southPoleObj = obj;
