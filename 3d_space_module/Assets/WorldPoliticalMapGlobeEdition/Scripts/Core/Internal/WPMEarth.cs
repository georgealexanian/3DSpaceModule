	...
	public partial class WorldMapGlobe : MonoBehaviour {
		...
		void UpdateMaterialBrightness () {
			if (earthRenderer == null || _earthRenderer.sharedMaterial == null)
				return;
			_earthRenderer.sharedMaterial.SetFloat ("_Brightness", _brightness);
			_earthRenderer.sharedMaterial.SetFloat ("_Contrast", _contrast);
			_earthRenderer.sharedMaterial.SetFloat ("_AmbientLight", _ambientLight);
		}
		...
		void UpdateMaterialsZWrite () {
			...
			if (outlineMat != null) {
				outlineMat.SetInt ("_ZWrite", zwriteValue);
			}
			...
			if (citiesNormalMat != null) {
				citiesNormalMat.renderQueue = zwriteValue == 1 ? RENDER_QUEUE_TRANSPARENT - 9 : RENDER_QUEUE_OPAQUE - 9;
			}
			if (citiesRegionCapitalMat != null) {
				citiesRegionCapitalMat.renderQueue = zwriteValue == 1 ? RENDER_QUEUE_TRANSPARENT - 9 : RENDER_QUEUE_OPAQUE - 9;
			}
			if (citiesCountryCapitalMat != null) {
				citiesCountryCapitalMat.renderQueue = zwriteValue == 1 ? RENDER_QUEUE_TRANSPARENT - 9 : RENDER_QUEUE_OPAQUE - 9;
			}
		...
		void DrawAtmosphere () {
					...
					if (glowEnabled) {
						if (_earthGlowScatter) {
							...
							if (skyRenderer.sharedMaterial != earthGlowMat) {
								...
								skyRenderer.transform.localScale = Misc.Vector3one * 1.12f;
							}
							if (_sun != null) {
								_earthScenicLightDirection = -_sun.forward;
							}
			...
			Shader.SetGlobalVector ("_WPM_GlobePos", new Vector4 (t.position.x, t.position.y, t.position.z, (t.localScale.x*0.5f) * (t.localScale.x * 0.5f)));
		}
