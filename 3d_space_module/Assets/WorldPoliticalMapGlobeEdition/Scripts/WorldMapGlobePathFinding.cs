...
namespace WPM {
	public partial class WorldMapGlobe : MonoBehaviour {
		...
		public List<int> FindPath (int cellIndexStart, int cellIndexEnd, int searchLimit = 0, bool includeStartingCell = false) {
			if (cellIndexStart < 0 || cellIndexStart >= cells.Length || cellIndexEnd < 0 || cellIndexEnd >= cells.Length)
				return null;
