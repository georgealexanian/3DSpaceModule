			...
			set {
				if (value != _earthInvertedMode) {
					...
					if (_earthInvertedMode) {
						...
						cam.transform.rotation = Misc.QuaternionZero;
						if (!UnityEngine.XR.XRSettings.enabled) {
							cam.fieldOfView = MAX_FIELD_OF_VIEW;
						}
					} else {
						cam.transform.position = transform.position + Vector3.back * lastRestyleEarthNormalsScaleCheck.z * 1.2f;
		...
		public void StraightenGlobe (float duration, bool keepLocationOnCenter) {
			...
			if (_earthInvertedMode) {
				if (keepLocationOnCenter) {
					...
					Vector3 v2 = cam.transform.forward;
				...
				if (keepLocationOnCenter) {
					...
					transform.Rotate (v2, -angle2, Space.World);
					Vector3 currentDestination = transform.InverseTransformVector (cam.transform.position - transform.position);
					flyToEndDestination = currentDestination;
				} else {
					Vector3 v1 = cam.transform.position - transform.position;
