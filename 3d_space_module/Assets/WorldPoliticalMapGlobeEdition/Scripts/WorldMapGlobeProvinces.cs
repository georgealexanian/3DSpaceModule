	...
	public partial class WorldMapGlobe : MonoBehaviour {
		...
		public int GetProvinceNearPoint (Vector3 spherePosition) {
			...
			for (int k = 0; k < _provinces.Length; k++) {
				float dist = FastVector.SqrDistanceByValue (_provinces [k].sphereCenter, spherePosition); // Vector3.SqrMagnitude (_provinces [k].sphereCenter - spherePosition);
				if (dist < minDist) {
					minDist = dist;
					provinceIndex = k;
				}
		...
		/// </summary>
		public void FlyToProvince (int provinceIndex, float duration, float zoomLevel) {
			if (provinceIndex < 0 || provinceIndex >= provinces.Length)
				return;
			FlyToLocation (_provinces [provinceIndex].sphereCenter, duration, zoomLevel);
		}
		...
		/// </summary>
		public void FlyToProvince (int provinceIndex, float duration, float zoomLevel, float bounceIntensity) {
			if (provinceIndex < 0 || provinceIndex >= provinces.Length)
				return;
			FlyToLocation (_provinces [provinceIndex].sphereCenter, duration, zoomLevel, bounceIntensity);
		}
		...
		public GameObject ToggleProvinceRegionSurface (int provinceIndex, int regionIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool temporary) {
			...
			bool isHighlighted = provinceHighlightedIndex == provinceIndex && provinceRegionHighlightedIndex == regionIndex;
			if (surf != null) {
