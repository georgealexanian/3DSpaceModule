...
namespace WPM {
	...
	public partial class WorldMapGlobe : MonoBehaviour {
		...
		[SerializeField]
		Color
			_outlineEdgeReliefColor = Color.black;

		/// <summary>
		/// Color for one of the right edges of the outline.
		/// </summary>
		public Color outlineEdgeReliefColor {
			get {
				return _outlineEdgeReliefColor;
			}
			set {
				if (value != _outlineEdgeReliefColor) {
					_outlineEdgeReliefColor = value;
					isDirty = true;
		...
		public bool CountryRename (string oldName, string newName) {
			...
			if (mountPoints==null) ReadMountPointsPackedString();
		...
		public int GetCountryNearPoint (Vector3 spherePosition) {
			...
			for (int k = 0; k < countryCount; k++) {
				...
				float dist = FastVector.SqrDistanceByValue (country.sphereCenter, spherePosition); // Vector3.SqrMagnitude (country.sphereCenter - spherePosition);
				if (dist < minDist) {
					minDist = dist;
					countryIndex = k;
				}
		...
		public void FlyToCountry (int countryIndex, float duration) {
			if (countryIndex < 0 || countryIndex >= countries.Length)
				return;
			FlyToLocation (countries [countryIndex].sphereCenter, duration, 0, _navigationBounceIntensity);
		}
		...
		public void FlyToCountry (int countryIndex, float duration, float zoomLevel) {
			if (countryIndex < 0 || countryIndex >= countries.Length)
				return;
			FlyToLocation (countries [countryIndex].sphereCenter, duration, zoomLevel, _navigationBounceIntensity);
		}
		...
		/// </summary>
		public void FlyToCountry (int countryIndex, float duration, float zoomLevel, float bounceIntensity) {
			if (countryIndex < 0 || countryIndex >= countries.Length)
				return;
			FlyToLocation (countries [countryIndex].sphereCenter, duration, zoomLevel, bounceIntensity);
		}
		...
		/// </summary>
		public void ToggleCountrySurface (string name, bool visible, Color color, Texture2D texture) {
			int countryIndex = GetCountryIndex (name);
			if (countryIndex < 0 || countryIndex >= countries.Length)
				return;
			for (int r = 0; r < countries [countryIndex].regions.Count; r++) {
				ToggleCountryRegionSurface (countryIndex, r, visible, color, texture, Misc.Vector2one, Misc.Vector2zero, 0);
			}
		}
		...
		public List<Country>GetVisibleCountries () {
			...
			for (int k = 0; k < countries.Length; k++) {
				...
				Vector3 center = transform.TransformPoint (country.sphereCenter);
		...
		public bool CountryTransferCountryRegion (int countryIndex, Region sourceCountryRegion) {
...
//																pc.Compute (PolygonOp.DIFFERENCE, sourceCountry.regions);
//												} else {
...
//												if (targetCountry.regions != null && targetCountry.mainRegionIndex >= 0 && targetCountry.mainRegionIndex < targetCountry.regions.Count) {
...
//																if (targetRegion != null && provinceRegion.Intersects (targetRegion)) {
...
//																				newCountryRegion.UpdatePointsAndRect (provinceRegion.latlon);
...
//												for (int r = 0; r < sourceCountry.regions.Count; r++) {
//																Region sourceCountryRegion = sourceCountry.regions [r];
//																for (int t = 0; t < targetCountry.regions.Count; t++) {
//																				Region region1 = targetCountry.regions [t];
//																				if (sourceCountryRegion.Intersects (region1)) {
//																								// Extract from source country - only if province is in the frontier or is crossing the country
//																								RegionMagnet (sourceCountryRegion, region1);
//																								pc = new PolygonClipper (sourceCountryRegion, region1);
//																								if (pc.OverlapsSubjectAndClipping ()) {
//																												pc.Compute (PolygonOp.DIFFERENCE, sourceCountry.regions);
//																								}
//																				}
