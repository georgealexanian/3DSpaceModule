...
//					if (_combineCityMeshes) DrawCities();
		...
		/// </summary>
		public bool FlyToCity (int cityIndex, float duration) {
			if (cityIndex < 0 || cityIndex >= cities.Count)
				return false;
			FlyToLocation (cities [cityIndex].unitySphereLocation, duration, 0, _navigationBounceIntensity);
			return true;
		}
		...
		public void FlyToCity (City city, float duration) {
			FlyToLocation (city.unitySphereLocation, duration, 0, _navigationBounceIntensity);
		}
		...
		public void FlyToCity (int cityIndex, float duration, float zoomLevel) {
			if (cityIndex < 0 || cityIndex >= cities.Count)
				return;
			FlyToLocation (_cities [cityIndex].unitySphereLocation, duration, zoomLevel, _navigationBounceIntensity);
		}
		...
		public void FlyToCity (int cityIndex, float duration, float zoomLevel, float bounceIntensity) {
			if (cityIndex < 0 || cityIndex >= cities.Count)
				return;
			FlyToLocation (_cities [cityIndex].unitySphereLocation, duration, zoomLevel, bounceIntensity);
		}
